...
const ImportLabelPopup: React.FC<IProps> = ({
    ...
    const resolveFormatType = (labelType: LabelType): AnnotationFormatType => {
        ...
        return possibleImportFormats.length === 1
            ? possibleImportFormats[0].type
            : null;
    };

    const [labelType, setLabelType] = useState(activeLabelType);
    const [formatType, setFormatType] = useState(
        resolveFormatType(activeLabelType)
    );
    ...
    return (
        <GenericLabelTypePopup
            ...
            disableAcceptButton={
                loadedImageData.length === 0 || !!annotationsLoadedError
            }
            rejectLabel={'Cancel'}
            onReject={onReject}
