...
import {LabelModeType, LabelType} from '../../data/enums/LabelType';
...
export class RectRenderEngine extends BaseRenderEngine {
    ...
    public mouseDownHandler = (data: EditorData) => {
        const isMouseOverImage: boolean =
        ...
        if (isMouseOverCanvas) {
            ...
            if (!!rectUnderMouse) {
                ...
                const anchorUnderMouse: RectAnchor =
    ...
    public mouseUpHandler = (data: EditorData) => {
        if (!!data.viewPortContentImageRect) {
            ...
            const activeLabelRect: LabelRect =
                LabelsSelector.getActiveRectLabel();
            ...
            if (
                ...
                this.addRectLabel(
                    RenderEngineUtil.transferRectFromImageToViewPortContent(
                        rect,
                        data
                    )
                );
            ...
            if (!!this.startResizeRectAnchor && !!activeLabelRect) {
                ...
                const scale: number =
                    RenderEngineUtil.calculateImageScale(data);
        ...
        if (
            ...
            if (isOverImage && !this.startResizeRectAnchor) {
                ...
                if (!!labelRect && !this.isInProgress()) {
                    if (
                        LabelsSelector.getHighlightedLabelId() !== labelRect.id
                    ) {
    ...
    private updateCursorStyle(data: EditorData) {
        if (
            ...
            const rectUnderMouse: LabelRect = this.getRectUnderMouse(data);
            const rectAnchorUnderMouse: RectAnchor =
    ...
    private addRectLabel = (rect: IRect) => {
        ...
        const imageData: ImageData = LabelsSelector.getActiveImageData();
        const labelRect: LabelRect = LabelUtil.createLabelRect(
            activeLabelId,
            rect
        );
        if (LabelsSelector.getActiveLabelMode() === LabelModeType.HUMAN) {
            const humanInfo: HumanInfo = {
                uuid: labelRect.id,
                id: imageData.humans.length,
                gender: LabelsSelector.getActiveGender(),
                type: LabelsSelector.getActiveHumanType(),
            ...
            const itemInfo: ItemInfo = {
                uuid: labelRect.id,
                id: imageData.items.length,
                humanId: LabelsSelector.getActiveHumanID(),
                gender: LabelsSelector.getActiveGender(),
                mainCategory: LabelsSelector.getActiveMainCategory(),
                subCategory: LabelsSelector.getActiveSubCategory(),
                color: LabelsSelector.getActiveColor(),
                pattern: LabelsSelector.getActivePattern(),
                styles: LabelsSelector.getActiveStyles()
            };
