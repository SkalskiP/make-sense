...
export class AutoRectUtil {
    public static getRatio(rect: IAutoRect): number {
        if (!rect) return null;

        return rect.width/rect.height
    }

    public static intersect(r1: IAutoRect, r2: IAutoRect) {
    ...
    public static isPointInside(rect: IAutoRect, point: IPoint): boolean {
    ...
    public static getRectWithCenterAndSize(centerPoint: IPoint, size: ISize): IAutoRect {
        return {
    ...
    public static fitInsideRectWithRatio(containerRect: IRect, ratio: number): IAutoRect {
        const containerRectRatio = AutoRectUtil.getRatio(containerRect);
    ...
    public static resizeRect(inputRect: IAutoRect, rectAnchor: Direction, delta): IRect {
        const rect: IAutoRect = {...inputRect};
        switch (rectAnchor) {
    ...
    public static translate(rect: IAutoRect, delta: IPoint): IAutoRect {
        return {
            ...rect,
            x: rect.x + delta.x,
            y: rect.y + delta.y
    ...
    public static expand(rect: IAutoRect, delta: IPoint): IAutoRect {
        return {
            x: rect.x - delta.x,
            y: rect.y - delta.y,
    ...
    public static scaleRect(rect:IAutoRect, scale: number): IAutoRect {
        return {
            x: rect.x * scale,
            y: rect.y * scale,
    ...
    public static mapRectToAnchors(rect: IAutoRect): RectAnchor[] {
        return [
    ...
    public static snapPointToRect(point: IPoint, rect: IAutoRect): IPoint {
        if (AutoRectUtil.isPointInside(rect, point))
            return point;
